    ...
    <div className="wizard-container">
      <div className="progress-bar-container">
        <div className="progress-bar">
          ...
          <div className={`progress-step ${step >= 4 ? 'active' : ''} ${step > 4 ? 'completed' : ''}`}>Salary</div>
          <div className={`progress-step ${step >= 5 ? 'active' : ''}`}>Complete</div>
        </div>
      </div>
      <div className="form-container">
        ...
        {step === 4 && <ReasonableSalaryForm onBack={prevStep} onNext={nextStep} />}
        {step === 5 && <SuccessMessage onNewTaxFile={resetStep} />}
      </div>
    </div>
  );
};
